...
function twoDecimals2($val){
    global $preciosredondos;
    $val = ($preciosredondos ? floor($val) : round($val, 2));
    return($val);
}
/*
	...
	<?=$analytics?>
		<header>
			<div class="content">
				<div class="shoppingcart">
					...
					<?=countProducts($cart)?> artículo(s) - <?=dotToComma(twoDecimals2($totalcostWOshipment))?>€
				</div>
		...
		<div class="fulltextbody">
			<div class="content productpage">
				...
				if($liq){?>
				<strong style="font-size: 16px; text-decoration: line-through; color: rgba(000,000,000,0.8);"><?=dotToComma(getProdInfo('cost', $id))?></strong> <strong style="font-size: 20px; color: red;"><?=dotToComma(twoDecimals2(getProdInfo('cost', $id) * $liqdiscount))?> &euro;</strong> <a class="addtocartbtn" href="<?=$baseurl?>/cart/?add=<?=$id?>">Añadir a la cesta</a><br>
				<span style="font-size: 12px;">¡Descuentos, descuentos! <?=$liqpercent?> de descuento aplicado en este producto :)</span>
				<?php } else { ?>
				<strong style="font-size: 20px;"><?=dotToComma(twoDecimals2(getProdInfo('cost', $id)))?> &euro;</strong> <a class="addtocartbtn" href="<?=$baseurl?>/cart/?add=<?=$id?>">Añadir a la cesta</a><br>
				<?php }

				} else {
					?>
				<strong style="font-size: 20px; color: rgba(000,000,000,0.3); font-weight: normal !important;"><?=dotToComma(twoDecimals2(getProdInfo('cost', $id)))?> &euro;</strong> <a class="addtocartbtn" href="#">SIN STOCK</a><br>
                ...
                <h2>Productos relacionados</h2><hr>
