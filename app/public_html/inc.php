...
// Precios redondos, cambiar "false" por "true" (sin comillas ¬¬)
...
function showCats($cat=NULL){
	...
	while($row = $result->fetch_object()){
		...
		$seoname = str_replace('á', 'a', $seoname);
		$seoname = str_replace('é', 'e', $seoname);
		$seoname = str_replace('í', 'i', $seoname);
		$seoname = str_replace('ó', 'o', $seoname);
		$seoname = str_replace('ú', 'u', $seoname);
		if($row->parent == NULL && $id != 7 && $id != 51 && $id != 53 && $id != 62 && $id != 91 && $id != 96){
	...
	if($cat == 96){
		echo "<a href=\"$baseurl/cat/96/Esta-de-moda/\" class=\"active\"><strong>Está</strong>de<strong>moda</strong></a>";
	} else {
		echo "<a href=\"$baseurl/cat/96/Esta-de-moda/\"><strong>Está</strong>de<strong>moda</strong></a>";
	}
	if($cat == 7){
...
if(strpos($cat, ' 7,') !== false){
	...
	$liqpercent = "10%";
			...
			$name = utf8_decodeAlt($row2->name);
			$seoname = seoName($name);
            if($liq){$name = "<strong>(-$liqpercent)</strong> $name";}
            $group = $row2->group;
			$cropname = croptxt($name, 40);
			$pictureglob = glob("./imgpr/$prodid/*.*");
			$picture = './img/nophoto.png';
...
function showSubCatProducts($array){
    ...
    global $baseurl;
        foreach($array as $prodgroup => $products){
            natsort($array[$prodgroup]);
            $arrayfinal = array_reverse($array[$prodgroup], false);
            
            foreach($arrayfinal as $id){
                $query = $mysqli->query("SELECT * FROM `products` where id = '$id'");
                $row2 = $query->fetch_object();
...
if(strpos($cat, ' 7,') !== false){
	...
	$liqpercent = "10%";
            ...
            $group = $row2->group;
			$stock = $row2->stock;
			if(count($pictureglob) > 0 && file_exists("./imgpr/$prodid/")){
				$picture = utf8_encode(str_replace(' ', '%20', $pictureglob[0]));
			}
		if($stock >=1){
			if($row2->new == 1){
...
*/
			if($liq == true){ $cost = $cost * 0.5; } else if($liq2 == true){ $cost = $cost * 0.7; } else if($liq3 == true){ $cost = $cost * 0.9; } else {
            ...
            if($preciosredondos && $cost < 1) $cost = 1;
...
function trespordos_total(){
    ...
    foreach($_SESSION['3x2_p'] as $prod => $cant){
        $price = getPrice($prod);
        if(isLiq($prod) == '50%'){$price = $price * 0.5;}elseif(isLiq($prod) == '30%'){$price = $price * 0.7;};
        $price = $price * $cant;
...
*/
			if($liq == true){ $cost = $cost * 0.5; } else if($liq2 == true){ $cost = $cost * 0.7; } else if($liq3 == true){ $cost = $cost * 0.9; }
